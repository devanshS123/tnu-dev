...
exports.getAssignedQuizV2WithQuestionsTry = functions.https.onRequest(async (req, res) => {
  try {
    // Extract fields from the request body
...
exports.getAssignedQuizV2WithQuestions = functions.https.onCall(async (data, context) => {
  try {
    if (!context.auth) {
    ...
    assignedQuizData.groupedQuestions = groupedQuestions;
...
exports.storeQuizData = functions.https.onCall(async (data, context) => {
  try {
    ...
    Object.entries(payload).forEach(([subject, topics]) => {
...
//     if (
//       providers.length === 1 &&
//       ['google.com', 'phone'].includes(providers[0])
//     ) {
//       matchedUsers.push({
//         uid: user.uid,
...
//     } else if (
//       providers.length === 1 &&
//       ['password', 'phone'].includes(providers[0])
//     ) {
//       matchedUsers.push({
//         uid: user.uid,
//         email: user.email || null,
...
//   // Accumulate users from next page
//   if (result.pageToken) {
//     const nextUsers = await fetchUsers(result.pageToken);
//     return matchedUsers.concat(nextUsers); // <- Accumulate
//   }

//   return matchedUsers;
// }

// exports.listIncompleteUsers = functions.https.onCall(async (req, res) => {
//   try {
    
//     const matchedUsers =  await fetchUsers();
...
//     return {
//       success: true, Msg: "Error fetching users",
//     };
//   }
// });

exports.deleteIncompleteUser = functions.https.onCall(async (data, context) => {
  const uid = data.uid;
  ...
  try {
    ...
    return {
      success: true,
      message: `User ${uid} was deleted successfully.`,
    };
...
// exports.listIncompleteUsers = functions.https.onCall(async (data, context) => {
//   try {
//     const unmatchedUsers = await fetchUsers();

//     console.log(`✅ Total unmatched users: ${unmatchedUsers.length}`);

//     return {
//       success: true,
//       data: {
//         totalUnmatched: unmatchedUsers.length,
//         sample: unmatchedUsers,
//       },
//     };
//   } catch (error) {
//     console.error('❌ Error fetching users:', error);
...
async function getAllAuthUserIds(nextPageToken) {
  ...
  if (listUsersResult.pageToken) {
    const nextPageUserIds = await getAllAuthUserIds(listUsersResult.pageToken);
    nextPageUserIds.forEach(id => userIds.add(id));
  }
...
async function getAuthUsersByIds(uidArray) {
  const userFetchPromises = uidArray.map(uid =>
    admin.auth().getUser(uid).catch(() => null) // Ignore missing users
  );
...
exports.listIncompleteUsers = functions.https.onCall(async (data, context) => {
  try {
    ...
    const [studentIds, userIds, authUserIds] = await Promise.all([
      getAllDocumentIds("zSystemStudents"),
      getAllDocumentIds("zSystemUsers"),
      getAllAuthUserIds()
    ]);
    // console.timeEnd('check 1')
    // console.time('check 2')

    const firestoreIdSet = new Set([...studentIds, ...userIds]);
    const missing = [...authUserIds].filter(uid => !firestoreIdSet.has(uid));
    const inactiveUser = await getAuthUsersByIds(missing);
    // console.timeEnd('check 2')


    return {
      success: true,
      inactiveUser: inactiveUser.map(user => ({
        uid: user.uid,
        email: user.email || null,
        displayName: user.displayName || null,
        phoneNumber: user.phoneNumber || null,
